...
DATA_FILE = "data.json"
users = {}
ADMIN_ID = "496526957"  # Замените на ваш реальный Telegram ID

def load_data():
    try:
        with open(DATA_FILE, "r") as f:
            loaded_users = json.load(f)
            # Очищаем устаревшие квесты при загрузке
            for user_id, data in loaded_users.items():
                if "current_quest" in data and data["current_quest"] is not None:
                    logger.warning(f"Обнаружен устаревший квест для {user_id}: {data['current_quest']}. Очищаем.")
                    data["current_quest"] = None
            return loaded_users
...
# Регистрация пользователя для статистики
def register_user(user_id):
    ...
    users[user_id]["last_active"] = asyncio.get_event_loop().time()
    save_data()

# Функция восстановления энергии
async def regenerate_energy(context: ContextTypes.DEFAULT_TYPE):
    for user_id in users:
        current_energy = users[user_id]["energy"]
        if current_energy < 100:
            new_energy = min(100, current_energy + 10)  # Восстанавливаем +10, но не больше 100
            users[user_id]["energy"] = new_energy
            logger.info(f"Энергия восстановлена для {user_id}: {current_energy} -> {new_energy}")
    save_data()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    register_user(user_id)  # Регистрируем пользователя
    ...
    msg = await send_with_retry(context.bot, chat_id, welcome_text, reply_markup=get_main_menu(user_id))
    if msg:
...
async def description(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    full_description = (
        ...
        "- *Энергия*: Квесты и бои тратят энергию (15-60), восстанавливай через отдых или жди (автовосстановление каждые 30 минут).\n"
...
async def quest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем и логируем состояние current_quest
    current_quest = users[user_id].get("current_quest")
    logger.info(f"Состояние current_quest для {user_id} перед созданием миссии: {current_quest}")
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id != ADMIN_ID:
        await context.bot.send_message(chat_id=chat_id, text="Эта команда доступна только администратору!")
        return
    
    total_users = len(users)
    stats_text = (
        f"📊 *Статистика бота TimeQuest* 📊\n\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    register_user(user_id)  # Регистрируем пользователя при любом сообщении

    if text == "Показать меню":
        if users.get(user_id, {}).get("current_quest"):
            msg = await send_with_retry(context.bot, chat_id, "Миссия ещё не закончена!", reply_markup=None)
            if msg:
                context.user_data["last_message_id"] = msg.message_id
        else:
...
async def update_quest_progress(bot, chat_id, message_id, title, total_time, user_id, context):
    ...
    while True:
        ...
        if remaining <= 0 or not users[user_id]["current_quest"]:
            logger.info(f"Прогресс-бар для квеста '{title}' завершён или прерван (user_id: {user_id})")
            break
    ...
    # Убеждаемся, что квест очищается после завершения
    if users[user_id]["current_quest"]:
        logger.info(f"Завершение квеста '{title}' для {user_id}")
        users[user_id]["exp"] += users[user_id]["current_quest"]["exp"]
        users[user_id]["coins"] += 10
        users[user_id]["quests_completed"] += 1
        msg = [f"Победа! +{users[user_id]['current_quest']['exp']} опыта, +10 монет"]
        ...
        save_data()
        await edit_with_retry(context.bot, chat_id, message_id, "\n".join(msg), reply_markup=get_main_menu(user_id))
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "create":
        ...
        save_data()  # Сохраняем данные сразу после создания квеста
        ...
        if msg:
            ...
            logger.error(f"Не удалось отправить сообщение о начале квеста для {user_id}")
            users[user_id]["current_quest"] = None  # Очищаем, если сообщение не отправлено
            save_data()
...
def main():
    global users
    users = load_data()
    app = Application.builder().token("7525183001:AAET8jlSxnxrldh9I5_lxxC-N7Rj3FpZ8BE").build()  # Замените на ваш токен
    ...
    # Настройка автоматического восстановления энергии каждые 30 минут
    if app.job_queue is None:
        logger.error("JobQueue недоступен! Установите 'python-telegram-bot[job-queue]' через pip.")
        return
    app.job_queue.run_repeating(regenerate_energy, interval=1800, first=1800)  # 1800 секунд = 30 минут
